...
module {
    ...
    public type Vol = { value0: Nat; value1: Nat; };
    ...
    public type KBar = {kid: Nat; open: Nat; high: Nat; low: Nat; close: Nat; vol: Vol; updatedTs: Timestamp}; // kid = ts_seconds / KInterval
...
public func natHash(n: Nat) : Hash.Hash {
    ...
    return Blob.hash(Blob.fromArray(bytes));
};
        ...
        while(Option.isSome(item.0) and i < depth_){
            ...
            switch(item.0){
                case(?(txid, orderPrice)){ 
                    ...
                    item := List.pop(ask);
                    if (prePrice == orderPrice.price){
                        i -= 1;
                    }else{
                        prePrice := orderPrice.price;
                    };
                };
                case(_){};
            };
    ...
    public func get(_ob: OrderBook, _txid: Txid, _side: ?OrderSide) : ?OrderPrice{
        switch(_side){
            case(?(#Buy)){
    ...
    public func putK(_kl: KLines, _ki: KInterval, _price: Nat, _vol: Vol): KLines{
        let kid = _now() / _ki;
        var kl = _kl;
                ...
                switch(Deque.popFront(klineData)){
                    case(?(kbar, klineTemp)){
                        if (kid > kbar.kid){ // new kbar
                            ...
                            klineData := Deque.pushFront(klineTemp, {kid = kid; open = kbar.open; high = Nat.max(kbar.high, _price); low = Nat.min(kbar.low, _price); close = _price; vol = {value0=kbar.vol.value0 + _vol.value0; value1=kbar.vol.value1 + _vol.value1}; updatedTs = _now()});
                        }else{
    ...
    public func putBatch(_kl: KLines, _filled: [OrderFilled], _UNIT_SIZE: Nat) : KLines {
        ...
        for (item in _filled.vals()){
            ...
            if (quantity > 0){
                ...
                kl := putK(kl, 60, price, {value0 = quantity; value1 = amount}); // 1min
                kl := putK(kl, 60*5, price, {value0 = quantity; value1 = amount}); // 5mins
                kl := putK(kl, 3600, price, {value0 = quantity; value1 = amount}); // 1h
                kl := putK(kl, 3600*24, price, {value0 = quantity; value1 = amount}); // 1d
                kl := putK(kl, 3600*24*7, price, {value0 = quantity; value1 = amount}); // 1w
                kl := putK(kl, 3600*24*30, price, {value0 = quantity; value1 = amount}); // 1m
            };
        };
        return kl;
    };
