...
use crate::pool_analytics::token_pricing::{get_anchor_usd,get_usd_price_from_pools};
...
// FIXED: get_token_price_us with better error handling
pub async fn get_token_price_usd(token: &StableToken) -> Result<f64, String> {
    let symbol = token.symbol().to_uppercase();

    match symbol.as_str() {
        "BELLA" => return Ok(7.89),
        "TOMMY" => return Ok(1.02),
        "USDT" | "CKUSDT" | "USDC" | "CKUSDC" => return Ok(1.0),
        _ => {}
    }

    if let Ok(p) = get_usd_price_from_pools(symbol.clone()).await {
        if p.is_finite() && p > 0.0 { //is finite make sure it's not nan or infinity
            return Ok(p);
        }
    }

    if let Ok(rate) = get_anchor_usd(symbol.as_str()).await {
 ...
 async fn token_amount_to_usd(amount: &Nat, token: &StableToken) -> Result<f64, String> {
    let price_usd = get_token_price_usd(token).await?; // Await the async call
    ...
    Ok((amount_f64 / decimals_factor) * price_usd)
}

#[update]
pub async fn calculate_pool_tvl(pool_id: u32) -> Result<PoolTVL, String> {
    // Phase 1: extract data synchronously
    let (pool, token_0, token_1) = POOLS.with(|pools| {
        TOKENS.with(|tokens| {
            ...
            let token_0 = tokens_map.get(&StableTokenId(pool.token_id_0))
                .ok_or("Token 0 not found")?.clone();
            let token_1 = tokens_map.get(&StableTokenId(pool.token_id_1))
                .ok_or("Token 1 not found")?.clone();

            Ok::<_, String>((pool, token_0, token_1))
        })
    })?;

    // Phase 2: do async work
    let total_balance_0 = nat_add(&pool.balance_0, &pool.lp_fee_0);
    let total_balance_1 = nat_add(&pool.balance_1, &pool.lp_fee_1);

    let token_0_value_usd = token_amount_to_usd(&total_balance_0, &token_0).await?;
    let token_1_value_usd = token_amount_to_usd(&total_balance_1, &token_1).await?;
...
pub async fn calculate_pool_tvl_internal(
    pool: &StablePool,
    tokens_map: &std::collections::BTreeMap<StableTokenId, StableToken>,
...
#[update]
pub async fn get_all_pools_tvl() -> Vec<PoolTVL> {
    // Step 1: Copy pools out
    let pools_vec: Vec<StablePool> = POOLS.with(|pools| {
        pools.borrow().iter().map(|(_, pool)| pool.clone()).collect()
    });

    // Step 2: Copy tokens out into a normal HashMap (sync only)
    let tokens_map: Vec<(StableTokenId, StableToken)> = TOKENS.with(|tokens| {
        tokens
            .borrow()
            .iter()
    ...
    // Rebuild a temporary in-memory map (so we don’t hold the RefCell across await)
    use std::collections::BTreeMap;
    let mut temp_tokens: BTreeMap<StableTokenId, StableToken> = BTreeMap::new();
    for (k, v) in tokens_map {
        temp_tokens.insert(k, v);
    }

    // Step 3: Process async
    let mut rows = Vec::new();
    for pool in pools_vec {
        if let Ok(row) = calculate_pool_tvl_internal(&pool, &temp_tokens).await {
            rows.push(row);
        }
    }
    // Step 4: Sort by TVL
...
#[update]
pub async fn calculate_pool_volume(pool_id: u32, hours: u64) -> Result<PoolVolume, String> {
    let current_time = get_time();
    let time_threshold = current_time - (hours * 60 * 60 * 1_000_000_000);

    // Step 1: copy pool, tokens, and transfers out synchronously
    let (pool, pool_transfers, tokens_vec) = TRANSFERS.with(|transfers| {
        TOKENS.with(|tokens| {
            POOLS.with(|pools| {
                ...
                let pool = pools_map
                    .get(&StablePoolId(pool_id))
                    .ok_or("Pool not found")?
                    .clone();

                // Collect relevant transfers
                ...
                for (_, transfer) in transfers_map.iter() {
                    if transfer.ts >= time_threshold
                        ...
                        && (transfer.token_id == pool.token_id_0
                            || transfer.token_id == pool.token_id_1)
                    {
                        pool_transfers.push(transfer.clone());
                    }
                }

                // Copy relevant tokens into Vec
                let mut tokens_vec = Vec::new();
                for (id, token) in tokens_map.iter() {
                    if id.0 == pool.token_id_0 || id.0 == pool.token_id_1 {
                        tokens_vec.push(token.clone());
                    }
                }

                Ok::<_, String>((pool, pool_transfers, tokens_vec))
            })
        })
    })?;

    // Step 2: async price calculations
    ...
    for (_ts, transfers_at_time) in timestamp_groups {
        ...
        for i in 0..transfers_at_time.len() {
            ...
            for j in (i + 1)..transfers_at_time.len() {
                ...
                if is_valid_pool_swap {
                    let (input_transfer, _) =
                        if transfer1.is_send { (transfer1, transfer2) } else { (transfer2, transfer1) };

                    // lookup token
                    if let Some(token) = tokens_vec
                        .iter()
                        .find(|t| t.symbol() == pool.name().split('_').next().unwrap_or(""))
                    {
                        if let Ok(swap_volume_usd) =
                            token_amount_to_usd(&input_transfer.amount, token).await
                        {
                            volume_usd += swap_volume_usd;
                            fees_usd += swap_volume_usd * (pool.lp_fee_bps as f64 / 10000.0);
                            transaction_count += 1;
                        }
                    }
    ...
    Ok(PoolVolume {
        pool_id,
        symbol,
        volume_24h_usd: if hours == 24 { volume_usd } else { 0.0 },
        volume_7d_usd: if hours == 168 { volume_usd } else { 0.0 },
        fees_24h_usd: if hours == 24 { fees_usd } else { 0.0 },
        fees_7d_usd: if hours == 168 { fees_usd } else { 0.0 },
        transactions_24h: if hours == 24 { transaction_count } else { 0 },
        transactions_7d: if hours == 168 { transaction_count } else { 0 },
    })
}
...
#[update]
pub async fn get_pool_metrics(pool_id: u32) -> Result<PoolMetrics, String> {
let tvl = calculate_pool_tvl(pool_id).await?;
let volume_24h = calculate_pool_volume(pool_id, 24).await?;
let volume_7d = calculate_pool_volume(pool_id, 168).await?;
// Combine volume data
...
#[update] // or #[query] but async
pub async fn get_all_pools_metrics() -> Vec<PoolMetrics> {
    let pool_ids: Vec<u32> = POOLS.with(|pools| {
        pools.borrow().iter().map(|(pool_id, _)| pool_id.0).collect()
    });

    let mut pool_metrics = Vec::new();
    for id in pool_ids {
        if let Ok(metrics) = get_pool_metrics(id).await {
            pool_metrics.push(metrics);
        }
    }

    pool_metrics.sort_by(|a, b| {
        b.tvl.tvl_usd
            .partial_cmp(&a.tvl.tvl_usd)
            .unwrap_or(std::cmp::Ordering::Equal)
    });
...
#[update]
pub async fn get_protocol_stats() -> ProtocolStats {
let all_metrics = get_all_pools_metrics().await;
...
#[ic_cdk::update]
pub async fn record_current_analytics() -> Result<String, String> {
let pools_tvl = get_all_pools_tvl().await;
let mut recorded_count = 0;

for pool_tvl in pools_tvl {
// Get volume for this pool
let volume_24h_usd = match calculate_pool_volume(pool_tvl.pool_id, 24).await {
